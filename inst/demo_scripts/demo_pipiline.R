...
### Originally created on : 2019/08/02
...
analysis.par$par.path<-par.path
par.name<-paste0("analysis_par_",lib.name, ".RData")
analysis.par$par.name<-par.name
save.image(file = paste0(par.path, par.name))
...
# 2. put library csv file with file name "lingyun6sample" in the analysis.par$out.dir.library folder.
# The 1st column must be gRNA name and 2nd column must be sequence. Suggested column ids are = c("id", "seqRNA", "Gene").
# In Yulab cluster, go to analysis.par$out.dir.library folder first,
# then run "cp /rgs01/project_space/yu3grp/FuncGenomics/yu3grp/training/ScreenBEAM2_Rpackage/demo_data/lingyun.csv ./"

# 3. Convert the csv library file to .fa file for blat. cd to analysis.par$out.dir.library folder,
# and run command "awk FNR-1 lingyun.csv | awk -F "," '{print ">"$1"\n"$2}' > lingyun.fa' to create fasta file for library.
# And check if the lingyun6sample.fa file has shRNA as fasta read name, and shRNA sequence as sequence.

# 4. meta data, please put meta data excel file in the analysis.par$out.dir.metadata folder.
# The meta data will be used after the raw count table is prepared. In Yulab cluster,
...
ScreenBEAM.mapping.QC(analysis.par,
                      QC.Rmd.path = "/rgs01/project_space/yu3grp/FuncGenomics/yu3grp/training/ScreenBEAM2_Rpackage/demo_run/mapping_QC_report.Rmd") # Will create QC report of mapping in analysis.dir$out.dir.QC folder

ScreenBEAM.mapping.QC(analysis.par,QC.Rmd.path="~/Documents/Github/ScreenBEAM2/inst/Rmd/mapping_QC_report.Rmd")


################### Add meta data, this pipeline need to be modified case to case, because sample names are different ####################
# pheno type data
meta<-openxlsx::read.xlsx(file.path(analysis.par$out.dir.metadata,"Metadata.xlsx"), sheet = 1)
dim(meta)
...
for(i in 1:dim(compare.pairs)[2]){
  ...
  de<-ScreenBEAM.rna.level(input.file, control.samples = control.samples, case.samples = case.samples,
                           ...
                           gene.columnId=2, do.log2=TRUE, do.normalization=FALSE, total=1e6, filterLowCount=TRUE,
...
for(i in 1:dim(compare.pairs)[2]){
  ...
  de<-ScreenBEAM.gene.level(input.file, control.samples = control.samples, case.samples = case.samples, data.type = "NGS",
                            control.groupname=control.groupname, case.groupname=case.groupname,
                            gene.columnId=2, do.normalization=FALSE, filterLowCount=TRUE,
                            filterBy='control',count.cutoff=4, rna.size = 6, sample.rna.time = 100, pooling = "partial",
                            method='Bayesian')
  names(de)[1]<-"geneID"
