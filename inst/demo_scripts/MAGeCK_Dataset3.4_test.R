...
demoOutput_dir <- '/home/dongxinran/project/screenBEAM2/dataset_result/D3.4/MAGeCKFlute/'
...
########## STEP1: CRISPR screen data can be processed with MAGeCK
#CRISPR screen data can be processed with MAGeCK (option A) or using MAGeCK-VISPR (option B). In a typical use case, CRISPR screen data are processed with MAGeCK (option A) step by step. If users want to perform QC and visualize the results, we recommend MAGeCK-VISPR (option B)
...
cmd <- sprintf('mageck test -k %s -t %s -c %s -n %s --cnv-norm %s --cell-line %s',
			count_file,sample_label_case, sample_label_control,output_prefix,cnv_file,cellline_name)
print(cmd)
system(cmd)
...
########## STEP2: Downstream analysis pipeline
###### A. Functional analysis for MAGeCK RRA results
gene_summary_file <- sprintf('%s.gene_summary.txt',output_prefix)
